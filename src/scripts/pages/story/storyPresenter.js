    ...
    try {
      ...
      if (result.ok) {
        ...
        await this._showMessage(
          result.message || "Gagal memuat cerita.",
          "error"
        );
      }
    } catch (error) {
    ...
    if (button) {
      button.addEventListener("click", async () => {
        ...
        const alreadySaved = saved.some(
          (item) => item.id === this.storyData.id
        );
        if (alreadySaved) {
          await Swal.fire({
            icon: "info",
            title: "Info",
            text: "Cerita ini sudah tersimpan.",
            confirmButtonColor: "#4caf50",
          });
          return;
        }

        // Tambahkan story ke localStorage
        saved.push(this.storyData);
